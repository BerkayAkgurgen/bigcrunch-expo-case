import Svg, {
  Mask,
  Path,
  Pattern,
  Use,
  G,
  Rect,
  Defs,
  Image,
} from "react-native-svg";

const MeetupIcon = () => {
  return (
    <Svg
      width="86"
      height="80"
      viewBox="0 0 86 80"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      xmlnsXink="http://www.w3.org/1999/xlink"
    >
      <Path
        opacity="0.8"
        d="M2 60.6299C2.44227 59.0512 3.02014 57.5518 4.23013 56.3693C6.31631 54.3422 8.78843 53.6228 11.5568 54.3506C14.3252 55.0784 16.1339 56.8928 16.8515 59.6685C18.1366 64.636 14.3794 69.2616 9.25156 69.0989C7.57734 69.038 5.97113 68.4212 4.68665 67.3461C3.40217 66.2709 2.51263 64.7985 2.15855 63.1616C2.11828 63.0342 2.06516 62.9112 2 62.7946L2 60.6299Z"
        fill="#F09E54"
      />
      <Path
        d="M19.8276 44.2004L29.7953 61.4576C31.1993 60.6547 32.5512 59.8601 33.9218 59.101C36.1144 57.8852 38.284 58.0854 40.0009 59.6349C42.3249 61.7203 42.1789 65.5284 39.6004 67.3031C37.9315 68.4459 36.1436 69.401 34.3849 70.4083C32.7765 71.3322 31.1847 72.2956 29.5137 73.0965C25.7252 74.9108 21.0355 73.916 18.1711 70.7733C22.4165 66.0602 23.4554 60.8674 20.2991 55.1699C18.9076 52.6674 16.7379 50.9615 14.0405 49.9438C11.4623 48.9564 8.62793 48.8588 5.98785 49.6664C4.59845 45.9439 6.02123 40.9868 9.35912 38.797C12.6198 36.6595 16.0349 34.7283 19.4562 32.8681C21.7844 31.6043 24.6487 32.5866 25.9422 34.8284C26.5896 35.9128 26.7937 37.205 26.5119 38.4361C26.2302 39.6671 25.4844 40.7421 24.4297 41.4372C22.9798 42.4278 21.4026 43.2599 19.8276 44.2004Z"
        fill="#F09E54"
      />
      <Mask
        id="mask0_3_57"
        style="mask-type:alpha"
        maskUnits="userSpaceOnUse"
        x="5"
        y="32"
        width="37"
        height="42"
      >
        <Path
          d="M19.8276 44.2004L29.7953 61.4576C31.1993 60.6547 32.5512 59.8601 33.9218 59.101C36.1144 57.8852 38.284 58.0854 40.0009 59.6349C42.3249 61.7203 42.1789 65.5284 39.6004 67.3031C37.9315 68.4459 36.1436 69.401 34.3849 70.4083C32.7765 71.3322 31.1847 72.2956 29.5137 73.0965C25.7252 74.9108 21.0355 73.916 18.1711 70.7733C22.4165 66.0602 23.4554 60.8674 20.2991 55.1699C18.9076 52.6674 16.7379 50.9615 14.0405 49.9438C11.4623 48.9564 8.62793 48.8588 5.98785 49.6664C4.59845 45.9439 6.02123 40.9868 9.35912 38.797C12.6198 36.6595 16.0349 34.7283 19.4562 32.8681C21.7844 31.6043 24.6487 32.5866 25.9422 34.8284C26.5896 35.9128 26.7937 37.205 26.5119 38.4361C26.2302 39.6671 25.4844 40.7421 24.4297 41.4372C22.9798 42.4278 21.4026 43.2599 19.8276 44.2004Z"
          fill="#F09E54"
        />
      </Mask>
      <G mask="url(#mask0_3_57)">
        <Rect
          opacity="0.3"
          x="57.1458"
          y="-4.11938"
          width="64.4019"
          height="115.638"
          transform="rotate(60 57.1458 -4.11938)"
          fill="url(#pattern0_3_57)"
        />
      </G>
      <Path
        d="M60.9054 19.1084C58.4534 21.4868 57.0999 24.3524 56.9371 27.7417C56.8084 30.2553 57.4637 32.7465 58.8124 34.8708C61.4537 39.1299 66.8869 41.8693 73.075 40.1978C73.8214 42.5095 73.6813 45.0166 72.6818 47.2312C71.7233 49.4168 70.0954 50.9894 68.0339 52.1575C65.3824 53.6595 62.7655 55.2224 60.1062 56.7127C56.8229 58.5581 53.0457 56.888 52.4712 53.3284C52.1563 51.3712 52.746 49.6617 54.4087 48.5211C55.9115 47.4905 57.5233 46.6216 59.1763 45.6271L49.2213 28.3722C47.8479 29.1619 46.4882 29.9403 45.1311 30.7226C43.351 31.7496 41.5431 31.8236 39.7666 30.7714C36.7709 28.9865 36.4739 24.4332 39.4076 22.5593C42.7962 20.3527 46.3145 18.3522 49.9436 16.5688C53.3122 14.9509 57.8287 15.7952 60.9054 19.1084Z"
        fill="#F09E54"
      />
      <Path
        d="M73.32 21.7218C76.7949 23.7281 77.9971 28.3776 75.9547 31.8693C73.8236 35.5145 69.2316 36.8405 65.7444 34.8223C62.1323 32.7369 60.8483 28.1581 62.7726 24.6874C64.9373 20.7797 69.8901 19.6838 73.32 21.7218Z"
        fill="#F09E54"
      />
      <Defs>
        <Pattern
          id="pattern0_3_57"
          patternContentUnits="objectBoundingBox"
          width="1"
          height="1"
        >
          <Use
            xlinkHref="#image0_3_57"
            transform="matrix(0.0394737 0 0 0.0241935 -1.39474 -0.516129)"
          />
        </Pattern>
        <Image
          id="image0_3_57"
          width="78"
          height="88"
          xlinkHref="data:image/png;base64,"
        />
      </Defs>
    </Svg>
  );
};

export default MeetupIcon;
